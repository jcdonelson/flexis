...
const IOPIN _baseiop_fb32[]={
		...
		// Note: These are the digital aliases for analog in pins.
...
// Note: You can not determine which one interrupted!!!
// See section 9.6.10
// KBIP0 = PTG0 (USER BUTTON)
// KBIP1 = PTG1 (RGB LED)
// KBIP2 = PTD2 D4 (KEYPAD PIN2)
// KBIP3 = PTD3 (KEYPAD PIN4)
// KBIP4 = ADP4
// KBIP5 = ADP5
// KBIP6 = PTG2
// KBIP7 = PTG3 (D2)
...
#if MCU_HCS08 == 0		
		// PTG3 KBI1P7 (D2)
		KBI1SC_KBACK = 1;  // Clear any false interrupt.
		KBI1PE |= 0x80;
...
{
	_contexts[ch] = context;
	attachInterrupt( ch,(PFNCALLBACK) cb,mode);
...
void ChannelInterruptHandler(byte ch)
{
	if( 0xFFFF == TPM1ChannelControl[ch])
	{
		...
		if(EdgeInterruptHandlers[ch] && _contexts[ch])
		{
		((PFNCALLBACKEX)EdgeInterruptHandlers[ch])(_contexts[ch]);
		}
		else if(EdgeInterruptHandlers[ch])
