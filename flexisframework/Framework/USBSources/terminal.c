...
extern void *rich_so;
#endif
static TRANSPORT_VTBL* vtbl;
static TRANSPORT_VTBL* ftdivtbl={
...
//    led_unknown_progress();
    ...
    if (ftdi_attached && ftdi_active) {
    	// call the transport layer print.
    	//vtbl->print_cbfn(buffer, length);
...
static void accumulate(char c)
{
    ...
    do {
        ...
        for (i = 0; i < LENGTHOF(keycodes); i++) {
            if (! strncmp(keycodes[i].keys, keys,(size_t) ki)) {
                // potential match
                ...
                switch (keycodes[i].code) {
                    ...
                    case KEY_DOWN:
                        if (keycodes[i].code == KEY_UP) {
                            if (hist_first) {
                                hist_out =(byte) HISTWRAP(hist_in-1);
                            } else {
                                hist_out =(byte) HISTWRAP(hist_out-1);
                            }
                        } else {
                            hist_out =(byte)  HISTWRAP(hist_out+1);
                        }
        ...
        for (i = 0; i < ki; i++) {
            if (isprint(keys[i])) {
                n =(int) strlen(command);
                if (n < sizeof(command)-1) {
                    memmove(command+cursor+1, command+cursor,(size_t) n+1-cursor);
...
static bool terminal_receive_internal(const byte *buffer, int length)
{
    int j;
    int x;
    int id;
    ...
    if (id != -1) {
        // forward packets
        x = splx(7);
        strncat(forward, (char *)buffer,(size_t) length);
        assert(strlen(forward) < sizeof(forward));
        boo =(bool) !! strchr((char *)forward, '\r');
        splx(x);
    ...
    for (j = 0; j < length; j++) 
    {
        ...
        if (!discard) 
        {
            if (buffer[j] == '\r') 
            {
            	// If this command is not exactly like the last command,
            	// put it into history.
                if (strcmp(history[HISTWRAP(hist_in-1)], command)) 
                {
                    strcpy(history[hist_in], command);
                    hist_in = (byte) HISTWRAP(hist_in+1);
                }
                
                // save extra for later
                assert(length >= j+1);
                strncpy(extra, (char *)buffer+j+1,(size_t) length-(j+1));
                extra[length-(j+1)] = '\0';
...
bool terminal_receive(const byte *buffer, int length)
{
    if (length) {
        // reply to local node
        terminal_txid = -1;
...
void terminal_command_discard(bool discard_in)
{
    discard = discard_in;
    terminal_getchar = 0;
}
...
void terminal_command_ack(bool edit)
{
    ...
    if (extra[0]) {
        // process it now
        x = splx(7);
        boo =(bool) terminal_receive_internal((byte *)extra,(int) strlen(extra));
        splx(x);
        if (! boo) {
...
void terminal_poll(void)
{
    ...
    if (terminal_echo && copy[0]) {
        terminal_print((byte *)copy, (int) strlen(copy));
        last = msecs;
    }
...
void terminal_register_transport(TRANSPORT_VTBL* vtbl)
{
	vtbl;
}
static void class_receive(int nodeid, int length, byte *buffer)
{
    if (length) {
...
static void class_print(int nodeid, int length, byte *buffer)
{
    int x;
    nodeid;
    
    x = splx(7);
    strncat(echo, (char *)buffer,(size_t) length);
    assert(strlen(echo) < sizeof(echo));
    splx(x);
}
