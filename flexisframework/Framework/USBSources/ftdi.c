...
//
// Properties C/C++ Builder/Setting/Coldfire Compile/Input 
// In user path : "K:\CWProjectsDEV\Framework\USBSources"
//#include "K:\CWProjectsDEV\Framework\USBSources/main.h"
#include "main.h"
...
#define ORGINAL_DESCRIPTOR  0
#define FTDI_VID  0x0403
#if ORGINAL_DESCRIPTOR == 1
#define FTDI_RID  0x0180
#define FTDI_PID  0xA660
#else
#define FTDI_PID  0xA667
#define FTDI_RID  0x0101
#endif
...
static const byte ftdi_string_descriptor[] = {
    ...
    38,  // length
    0x03,  // string descriptor
    'w', 0, 'w', 0, 'w', 0, '.', 0, 'F', 0, 'i', 0, 'r', 0, 'e', 0, 'b', 0, 'i', 0, 'r', 0, 'd', 0,'3', 0,'2', 0, '.', 0, 'c', 0, 'o', 0, 'm', 0,

    22,  // length
    0x03,  // string descriptor
    'F', 0, 'i', 0, 'r', 0, 'd', 0, 'b', 0, 'i', 0, 'r', 0, 'd', 0,'3', 0,'2', 0,
#endif
};
...
void ftdi_print(const byte *buffer, int length)
{
    int m;
    int x;
...
static int ftdi_control_transfer(struct setup *setup, byte *buffer, int length)
{
#if SODEBUG
...
static int ftdi_bulk_transfer(bool in, byte *buffer, int length)
{
    if (! in) 
    {
        discard = false;
...
void ftdi_register(ftdi_reset_cbfn reset)
{
    ...
    // Resister call back functions 
    usb_register(ftdi_reset, 
    		      ftdi_control_transfer, 
    		      ftdi_bulk_transfer);
