...
usb_device_default()
{
    ...
    memset(bdts, 0,(size_t) BDT_RAM_SIZE);
    memset(endpoints, 0, sizeof(endpoints));
...
usb_device_enqueue(int endpoint, bool tx, byte *buffer, int length)
{
    ...
    if (tx != (bool)-1) {
        ...
        bdt->buffer = (byte *)TF_LITTLE(KVA_TO_PA((uint32)buffer));
        flags =(int) TF_LITTLE((uint32)bdt->flags);
        assert(! (flags & BD_FLAGS_OWN));
        assert(length <= endpoints[endpoint].packetsize);
        f = TF_LITTLE((uint32)(BD_FLAGS_BC_ENC(length)|
        		BD_FLAGS_OWN|
        		endpoints[endpoint].toggle[tx]|
        		BD_FLAGS_DTS));
        bdt->flags =(int) f;
/*
        bdt->flags = TF_LITTLE(BD_FLAGS_BC_ENC(length)|
        		BD_FLAGS_OWN|
        		endpoints[endpoint].toggle[tx]|
        		BD_FLAGS_DTS);
*/        		
...
interrupt VectorNumber_Vusb void usb_isr(void)
{
#if ! STICKOS || STICKOSPLUS
    int e;
#endif
...
//    assert((usb_in_ticks = ticks) ? true : true);
    ...
    if (MCF_USB_OTG_INT_STAT & MCF_USB_OTG_INT_STAT_TOK_DNE) {
        ...
        endpoints[endpoint].bdtodd[tx] = (byte) ! endpoints[endpoint].bdtodd[tx];

        bdt = MYBDT(endpoint, tx, odd);

        flags =(int) TF_LITTLE((uint32)bdt->flags); // JCD 3
        assert(! (flags & BD_FLAGS_OWN));
        ...
        if (pid == TOKEN_SETUP) {
            ...
            if (! (setup->requesttype & 0x60)) {
                value =(short) TF_LITTLE(setup->value);
                if (setup->request == REQUEST_GET_DESCRIPTOR) {
                    ...
                    if ((value >> 8) == DEVICE_DESCRIPTOR) {
                        ...
                        memcpy(endpoints[endpoint].data_buffer, device_descriptor,(size_t) endpoints[endpoint].data_length); 
                        ...
                        memcpy(endpoints[endpoint].data_buffer, configuration_descriptor,(size_t) endpoints[endpoint].data_length); 
                        ...
                        if (i != -1) {
                            ...
                            memcpy(endpoints[endpoint].data_buffer, string_descriptor+j,(size_t) endpoints[endpoint].data_length);
                        }
                    ...
                    if (setup->request == REQUEST_CLEAR_FEATURE) {
                        ...
                        if (setup->requesttype == 0x02 && ! value) {
                            endpoint2 = (int) TF_LITTLE(setup->index) & 0x0f; // JCD 4 
                            assert(endpoint2);
...
XXX_SKIP2_XXX:;
            ...
            data = (byte *)TF_LITTLE((uint32)bdt->buffer); // JCD 2 
            ...
            assert(pid == TOKEN_IN || pid == TOKEN_OUT);
            data = (byte *)TF_LITTLE((uint32)bdt->buffer);

            // we just received or sent data from or to the host
            assert(bulk_transfer_cbfn);
            bulk_transfer_cbfn((bool)(pid == TOKEN_IN), data, bc);
        }
...
/*
 * This is called by ftdi_register
