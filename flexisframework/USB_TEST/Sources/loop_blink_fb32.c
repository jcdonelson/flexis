...
int bl_counter = 0;	// Determine when 1/2 a second is up.
int blink_state=0;	// Toggles with XOR

void RTCTickCallback(void)
{
    ++bl_counter;		// Increment the 1 ms counter.
    ++msecs;
    if(500 == bl_counter )	// 1/2 second yet ?
    {
        bl_counter = 0;            // reset the counter so we start over
