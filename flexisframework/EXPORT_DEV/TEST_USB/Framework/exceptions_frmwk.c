/*
 *  EXCEPTIONS_FRMWK.C
 *  Generic exception handling for ColdFire processors
 *  Modified to ignore exceptions from HALT instruction
 *  as the debug console uses this.
 *
 *  $Rev:: 71                        $:
...
#include "exceptions_frmwk.h"
//#include "startcf.h"
#include "CONFIG.H"
extern asm __declspec(register_abi) void _startup(void);
...
void mcf_exception_handler(void *framepointer) 
{
   ...
   dword* pc = (dword*) ((unsigned long *)framepointer+1);
   word instruction = 0;
   ...
   case 4:
      ...
      // Special code to handle the HALT instruction when not debugging.
      instruction = *((word*)stackFramePC );
      if(instruction == 0x4AC8) // If we get here with a halt instruction, not being debugged.
    	  *(pc)+= 2;            // so skip the instruction and continue.
      stackFramePC       = MCF5XXX_SF_PC(framepointer);
